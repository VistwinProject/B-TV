...
import { scenePosition } from '../timing.js'
const SPIN_MS = 7000

const isNum = (v) => /^[\d.]+$/.test(String(v))
const dec = (v) => (String(v).split('.')[1]?.length || 0)
...
export default function SceneBento({ persona, mode = 'play', onComplete }) {
  ...
  // ── 情境語音 ────────────────────────────────────────────────────────────────
  // 五個情境各一段配音(public/voice/scene-<id>.mp3)。換情境時 SceneBento 不 remount
  // (房子/影片要保活)→ 依 persona.id 重跑:先停掉上一段,再播這一段。
  // 沒有配音檔時退回瀏覽器語音唸下面這句(目前語音關閉 → 靜音,不影響流程)。
  useEffect(() => {
    speak(`${persona.label}。${persona.line}。`, { cue: `scene-${persona.id}` })
    return () => cancelSpeech()
  }, [persona.id, persona.label, persona.line])

  const order = DIMENSION_ORDER
  const { beat, dim, completed } = scenePosition(performance.now() - startRef.current, order.length)
  const curK = order[dim]

  useEffect(() => {
    if (mode === 'play' && completed) onComplete?.() // 走完一輪 → loop
  }, [completed, mode, onComplete])
