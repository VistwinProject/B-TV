...
export function createHomeFixtures(scene){
  ...
  for(const id of Object.keys(SCENE_VIEWS)){
    ...
    function floorLamp(x,z){
      shape(new T.CylinderGeometry(.17,.19,.04,18),[x,.3,z]);rod([x,.32,z],[x,1.8,z],.022)
      shape(new T.CylinderGeometry(.18,.29,.3,20,1,true),[x,1.92,z],true);wash([x,.28,z],1.6,1.7)
    }
    if(id==='anti-aging'){
      sconce(-.85);sconce(.55)
    ...
    sets.push({id,group,ink,fill,body,glow,weight:0})
  }
  return {
    tick(dt,id,state){for(const s of sets){s.weight=T.MathUtils.lerp(s.weight,id===s.id?1:0,1-Math.exp(-dt*2.5));s.group.visible=s.weight>.002;s.ink.opacity=s.weight*.85;s.ink.color.set(state.daylight>.5?'#555555':'#dddddd');s.body.opacity=s.weight*.23;s.fill.opacity=s.weight*(.07+state.brightness*.48);s.fill.color.copy(state.color);s.glow.uniforms.tint.value.copy(state.color);s.glow.uniforms.strength.value=s.weight*state.brightness*.5}},
