...
export default function App() {
  ...
  const { analyser, audioStatus, play } = useNarration(playback, issue, editing)
  ...
  switch (state.screen) {
    ...
    case 'experience': view = <Experience person={content.people.find(p => p.id === state.person)} dimension={state.dimension} />; break
    case 'narration': view = <Narration analyser={analyser} audioStatus={editing ? "idle" : audioStatus} onPlay={play} />; break
    case 'farewell': view = <Farewell analyser={analyser} />; break
    default: view = <Welcome />
  }
  const connectionLabel = connection === 'local' ? '本機預覽' : connection === 'connected' ? (reader ? '讀卡機就緒' : '等待讀卡機') : '連線中'
  return <main className="exhibition" data-screen={state.screen} data-editing={editing} style={theme}>
