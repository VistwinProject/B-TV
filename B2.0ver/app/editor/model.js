...
export function makeDefaults(people, house) {
  return {schema:1,theme:{bgStart:'#ffffff',bgMid:'#eef5ff',bgEnd:'#d7e6fa',haze:'#ffffff',title:'#1b3f8f',subtitle:'#6a8cc8',hint:'#2a56a8',core:'#ffffff',inner:'#57a8ff',glow:'#2d9afb'},beam:{points:[[-11,98],[59.5,83.5],[108,-24]],widths:[28,9,7],glow:1,edge:1},house:{columns:[.873,.467,.947,.878,1.553,1.282],rows:[1,.904,1.096,1],panels:Object.fromEntries(['intro',...house.map(m=>m.key),'claim'].map(key=>[key,{color:['intro','claim'].includes(key)?'#a2d0f8':'#ffffff',rim:!['intro','claim'].includes(key)}]))},scenes:Object.fromEntries(people.map(p=>[p.id,{columns:[...p.columns],painHeight:p.painHeight,homeView:{zoom:1,x:0,y:0},colors:[...p.colors]}])),material:{background:'key',panel:'flat',alpha:.72,wash:0,blur:0,gloss:.5,refract:'off'}}
}
...
export function normalizeDesign(value, defaults) {
  const walk=(v,b,path=[])=>{
    ...
    if(typeof b==='number'){
      ...
      let lo=0,hi=100
      if(path.includes('columns')||path.includes('rows'))[lo,hi]=[.15,5]
    ...
    const options={background:['key','white','video'],panel:['flat','glass'],refract:['off','soft','strong']}
