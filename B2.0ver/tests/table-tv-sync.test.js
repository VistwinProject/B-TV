...
test('narration completion fades to choices and rejects an older invitation completion',()=>{
 ...
 assert.equal(advancePresentation(tv,{action:'clock',now:14999}).transition,null)
 tv=advancePresentation(tv,{action:'clock',now:15000})
 assert.equal(tv.screen,'overview');assert.equal(tv.transition.target.screen,'choose')
 tv=advancePresentation(tv,{action:'clock',now:15250})
