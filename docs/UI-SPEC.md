...
### 5-3 情境牆 1-5 各格顏色（正式記錄）

定案值寫在 [`src/sceneColorStore.js`](../src/sceneColorStore.js) 的 `SCENE_DEFAULTS` —— **隨程式碼走**，
換電腦、清 localStorage、重新 clone 都一樣。編輯模式（`E`）挑的色只存在該台瀏覽器的
`localStorage.sceneColors`，屬於現場微調的暫時覆寫；要固化就把值抄進這張表
（面板的「複製 CSS」會把目前挑的色一起輸出）。

| 面板 | 居家抗老 | 兒童免疫 | 在宅樂齡 | 孕婦照護 | 數位遊牧 |
|---|---|---|---|---|---|
| 數據強調（長條/區間/波形） | `#fa864d` | `#e04b64` | `#bfd71b` | `#628e6b` | `#7cc8f0` |
| 痛點卡 · 右上 | `#fa864d` | `#e04b64` | `#bfd71b` | `#628e6b` | `#7cc8f0` |
| 解方卡 · 右下 | `#fa864d` | `#e04b64` | `#bfd71b` | `#628e6b` | `#7cc8f0` |
| 全健築指數 · 中欄 | 不平塗 | 不平塗 | 不平塗 | 不平塗 | 不平塗 |
| 左欄第 1 張 | 不平塗 | 不平塗 | 不平塗 | 不平塗 | 不平塗 |
| 左欄第 2 張 | `#17ab54` | `#549a60` | `#f96224` | `#5040ee` | `#3cb3a7` |
| 左欄第 3 張 | 不平塗 | 不平塗 | 不平塗 | 不平塗 | 不平塗 |

「不平塗」= `null`，維持半透明白 `rgba(255,255,255,0.13)`，讓背後的光透出來。

上表是**參考色原值**。實際鋪在卡片上會再經過兩道：

1. `fillColor()` 混 20% 暖灰 `#4a4a30`（參考色原值亮度偏高，白字會糊）
2. 卡片再取 72% alpha（`color-mix(… 72%, transparent)`），讓背景的光透出來

混色後的實際平塗值：

| 情境 | 痛點/解方（accent2 → 平塗） | 左欄第 2 張（accent3 → 平塗） |
|---|---|---|
| 居家抗老 | `#fa864d` → `#d77a47` | `#17ab54` → `#21984d` |
| 兒童免疫 | `#e04b64` → `#c24b5a` | `#549a60` → `#528a56` |
| 在宅樂齡 | `#bfd71b` → `#a8bb1f` | `#f96224` → `#d65d26` |
| 孕婦照護 | `#628e6b` → `#5d805f` | `#5040ee` → `#4f42c8` |
| 數位遊牧 | `#7cc8f0` → `#72afca` | `#3cb3a7` → `#3f9e8f` |
